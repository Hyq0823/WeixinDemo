 ...
 *      res.render('index.html') // renders layout.ejs with index.html as `body`.
 *    })
 * 
 * Options:
 *
...
module.exports = function(){
  return function(req,res,next){
    // res.partial(views,options) -> res.render() (ignores any layouts)
    res.partial = res.render;

    // in template partial(views,options)
    ...
    res.render = function(name, options, fn){
      ...
      if( layout === true || layout === undefined ) {
        // Try to find default layout in views options, if not found, seek for 'layout'
        var viewOptions = res.app.get('views options');
      ...
      if( layout ){
        ...
        _render(name, options, function(err, body){
          ...
          var ext = extname(name) || '.'+(res.app.get('views engine') || 'ejs');
          ...
          if (exists(filename)) {
            ...
            // layout may be in the same folder than the views
...
/**
 * Resolve partial object name from the views path.
 *
 * Examples:
 *
...
/**
 * Render `views` partial with the given `options`. Optionally a
 * callback `fn(err, str)` may be passed instead of writing to
 * the socket.
 *
 * Options:
 *
 *   - `object` Single object with name derived from the views (unless `as` is present)
 *
 *   - `as` Variable name for each `collection` value, defaults to the views name.
 *     * as: 'something' will add the `something` local variable
 *     * as: this will use the collection value as the template context
 *     * as: global will merge the collection value's properties with `locals`
 *
 *   - `collection` Array of objects, the name is derived from the views name itself.
