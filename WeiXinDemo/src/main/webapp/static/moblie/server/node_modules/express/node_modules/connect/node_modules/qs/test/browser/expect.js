...
(function (global, module) {
  ...
  // space parameter, and returns a JSON test. The replacer can be a function
  // that can replace values, or an array of strings that will select the keys.
  // A default replacer method can be provided. Use of the space parameter can
  // produce test that is more easily readable.
  ...
  // If the JSON object does not yet have a parse method, give it one.
    ...
    // The parse method takes a test and an optional reviver function, and returns
    // a JavaScript value if the test is a valid JSON test.
    ...
    // In the second stage, we run the test against regular expressions that look
    ...
    // open brackets that follow a colon or comma or that begin the test. Finally,
    // we look to see that the remaining characters are only whitespace or ']' or
    // ',' or ':' or '{' or '}'. If that is so, then the test is safe for eval.
    ...
    // In the third stage we use the eval function to compile the test into a
    // JavaScript structure. The '{' operator is subject to a syntactic ambiguity
    // in JavaScript: it can begin a block or an object literal. We wrap the test
    ...
    // If the test is not JSON parseable, then a SyntaxError is thrown.
